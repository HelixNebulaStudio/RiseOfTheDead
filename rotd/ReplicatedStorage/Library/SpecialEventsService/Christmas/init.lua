...
local SpecialEventPackage = {};
--==

function SpecialEventPackage.GetPlayerGiftDropItemId(player, param): (string?, number?)
	param = param or {};
...
function SpecialEventPackage.onRequire()
    ...
    task.spawn(function()
        ...
        shared.modNpcs.spawn2{
            Name = "Mr. Klaws";
            CFrame = mrKlawsSpawn.CFrame * CFrame.Angles(0, math.rad(-90), 0) + Vector3.new(0, 2.3, 0);
        };
    end)
            ...
            task.spawn(function()
                ...
                while true do
                    if workspace.Interactables:FindFirstChild("TreelumSapling") == nil then
                        ...
                        local saplingInteractConfig = modInteractables.createInteractable("Button");
                        saplingInteractConfig:SetAttribute("_Id", "WinterTreelumSapling");
                        saplingInteractConfig:SetAttribute("_EquipItemId", "shovel");
                        saplingInteractConfig:SetAttribute("_Animation", "shoveling");
                        saplingInteractConfig:SetAttribute("_InteractDuration", 2);
                        saplingInteractConfig:SetAttribute("_Label", "Dig");
                        saplingInteractConfig.Parent = new;
    ...
    shared.modEventService:OnInvoked("Crates_BindSpawn", function(eventPacket: EventPacket, ...)
        local players = eventPacket.Players;
        local newPrefab, interactable: InteractableInstance = ...;

        task.spawn(function()
            local storageId = interactable.Values.StorageId;
            if storageId == nil then
                Debugger:Warn("Missing storageId.");
                return;
            end

            local lvlRequired = interactable.Values.LevelRequired or 1;
            ...
            for _, player in pairs(players) do
                ...
                local crateStorage: Storage = storages[storageId];
                if crateStorage == nil then continue end;

                local pickItemId, spawnAmount = SpecialEventPackage.GetPlayerGiftDropItemId(player, {
                    MaxQuantity = math.ceil(lvlRequired/65);
                });

                if pickItemId == nil then continue end;
    ...
    shared.modEventService:OnInvoked("NpcComponent_BindDropReward", function(eventPacket: EventPacket, ...)
        ...
        local dropRewardComp, spawnCf: CFrame = ...;

        local npcClass = dropRewardComp.NpcClass;
        ...
        task.spawn(function()
            ...
            local _, _, dropTable = SpecialEventPackage.GetPlayerGiftDropItemId(killerPlayer);
    ...
    shared.modEventService:OnInvoked("Generic_BindTrigger", function(eventPacket: EventPacket, ...)
        ...
        local triggerId, interactable: InteractableInstance = ...;
        if triggerId ~= "WinterTreelumSapling" then return end;

        local interactPart = interactable.Part;
        local spawnPosition = interactPart.Position;
        ...
        if workspace.Entity:FindFirstChild("Winter Treelum") == nil then
            ...
            game.Debris:AddItem(interactPart.Parent, 0);
            task.wait(1);
            
            shared.modNpcs.spawn2{
                Name = "Winter Treelum";
                CFrame = CFrame.new(spawnPosition + Vector3.new(0, 4, 0)) * CFrame.Angles(0, math.rad(math.random(0, 360)), 0);
    ...
    local modMinesweeper = shared.require(game.ReplicatedStorage.Library.Minigames.Minesweeper);
    ...
    function remoteFrostivus.OnServerInvoke(player, action, packet)
        ...
        elseif action == "uncoverminesweeper" then
            ...
            if storageItem then
                ...
                shared.Notify(player, `{itemLib.Name} removed from {storage.Name}.`, "Negative");
            end
