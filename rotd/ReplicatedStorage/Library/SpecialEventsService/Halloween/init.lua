...
function SpecialEventPackage.LoadSlaughterfestDialogues(trickNpcList, treatNpcList)
    ...
    for a=1, #trickNpcList do
        ...
        local fulfillLists = SpecialEventPackage.SlaughterfestGetCandyTrade(npcName);
...
function SpecialEventPackage.SlaughterfestGetCandyTrade(npcName)
    ...
    if npcName == nil then
        SpecialEventPackage.LoadSlaughterfestDialogues(tradeNpcList, treatNpcList);
        return;
    end
...
function SpecialEventPackage.onRequire()
    ...
    shared.modProfile.OnProfileLoad:Connect(function(player, profile)
        ...
        if activeSave.Storages and activeSave.Storages.HalloweenCauldron == nil then
            activeSave.Storages.HalloweenCauldron = modStorage.new("HalloweenCauldron", "HalloweenCauldron", player);

            local cauldronStorage: Storage = activeSave.Storages.HalloweenCauldron;
        ...
        SpecialEventPackage.SlaughterfestGetCandyTrade();
    end)
    ...
    function SpecialEventPackage.LoadCache()
        if SpecialEventPackage.Cache.LoadTick and tick()-SpecialEventPackage.Cache.LoadTick <= 5 then return end;
        SpecialEventPackage.Cache.LoadTick = tick();
        
        local s, e = pcall(function()
            local rawCandyPool = halloweenMemory:GetAsync("CandyPool");
            SpecialEventPackage.Cache.CandyPool = tonumber(rawCandyPool) or 0;
        end)
    ...
    function remoteHalloween.OnServerInvoke(player, packet)
        ...
        local rPacket = {};
        rPacket.CommunityContributions = SpecialEventPackage.Cache.CandyPool
        
        local candyData = loadCandyData(player);
        
        SpecialEventPackage.LoadCache();
    ...
    task.spawn(function()
        ...
        if EventSpawns then
            ...
            local cauldronInteractConfig = modInteractables.createInteractable("HalloweenCauldron");
            cauldronInteractConfig.Parent = newCauldron;

            newCauldron.Parent = workspace.Environment;
            newCauldron:SetPrimaryPartCFrame(cauldronSpawn.WorldCFrame);
        end
        
        shared.modEventService:OnInvoked("Tools_BindFoodTool", function(event: EventPacket, ...) 
            local player: Player? = event.Player;
            if player == nil then return end;

            local action, toolHandler: ToolHandlerInstance = ...;
            if action ~= "Consume" then return end;

            local itemId = toolHandler.EquipmentClass.ItemId;
            if not (itemId == "wickedtaffy" or itemId == "cherrybloodbar" or itemId == "spookmallow" or itemId == "eyeballgummies" or itemId == "zombiejello") then
                return;
            end
            ...
            if mission and mission.Type == 1 and mission.SaveData and mission.SaveData.List and table.find(mission.SaveData.List, itemId) then
