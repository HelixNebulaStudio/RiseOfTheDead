...
function toolPackage.InputEvent(handler: ToolHandlerInstance, inputData: ToolInputData)
	if inputData.InputState ~= "End" or inputData.KeyIds.KeyFire == nil then return end;

	local configurations = handler.EquipmentClass.Configurations;
	local properties = handler.EquipmentClass.Properties;

	if RunService:IsClient() then
		local window: InterfaceWindow = modClientGuis.getWindow(configurations.RoleplayStateWindow);
