...
function WorldEvent.Initialize(worldEventsService)
	...
	shared.modEventService:OnInvoked("Generic_BindTrigger", function(event: EventPacket, ...)
		...
		task.spawn(function()
			...
			if triggerId == nil or interactable == nil or interactable.Part == nil then return end;
			
			local model: Model = interactable.Part.Parent :: Model;
			...
			task.spawn(function()
				shared.modEventService:ServerInvoke("Generic_BindTrigger", {ReplicateTo={player}}, "SafehomeBreach_RepairWall");
			end)
...
function WorldEvent.Start()
	...
	if modBranchConfigurations.IsWorld("Safehome") then
		shared.Notify(game.Players:GetPlayers(), "Your safehome is getting breached! Quickly fight off the horde and refortify.", "Important");
	else
	...
	task.delay(duration, function()
		shared.Notify(game.Players:GetPlayers(), "The safehouse breach is now over, walls and fences are patched up.", "Important");
		
		task.spawn(function()
			shared.modEventService:ServerInvoke("Generic_BindTrigger", {}, "SafehomeBreach_EventEnd");
		end)
					...
					BindSetup = function(npcClass: NpcClass)
						...
						npcClass.HealthComp.OnIsDeadChanged:Connect(function(isDead)
							...
							for a=#WorldEvent.Enemies, 1, -1 do
								if WorldEvent.Enemies[a] == npcClass then
						...
						if targetHandlerComp and closestPlayer and closestPlayer.Character then
							targetHandlerComp:AddTarget(closestPlayer.Character);
						end
					end
				};
