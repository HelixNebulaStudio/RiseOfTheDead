...
return function(npc, spawnPoint)
	...
	function self.Initialize()
		...
		-- Targetable configuration removed - no longer supported
		...
		if self.Enemy and self.Enemy.Humanoid and self.Enemy.Humanoid.Health > 0 then
			...
			if self.Enemy.Distance <= (self.Properties.TargetableDistance or 50) then
				if self.WieldComp.ToolHandler == nil then
					self.WieldComp:Equip{ ItemId = self.Properties.WeaponId };
					-- MinBaseDamage configuration no longer needed
				end
				...
				repeat
					...
					if self.Enemy and self.Enemy.Humanoid and self.Enemy.Humanoid.RootPart and self.Enemy.Humanoid.Health > 0 then
						...
						local equipmentClass: EquipmentClass? = self.WieldComp.EquipmentClass;
						if equipmentClass and not equipmentClass.Properties.Reloading and targetIsInVision then
							...
							local direction = (self.Enemy.Humanoid.RootPart.Position - self.RootPart.Position).Unit;
							...
							if self.WieldComp.EquipmentClass and tick()-self.LostVisionOfTargetTick >= math.random(20,30)/10 then
								self.WieldComp.Controls.Mouse1Down = false;
							end
