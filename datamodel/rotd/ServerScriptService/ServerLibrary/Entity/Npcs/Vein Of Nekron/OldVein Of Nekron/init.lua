...
local ZombieModule = script.Parent.OldZombie;
--== Modules Warn: Don't shared.require(Npc)

-- local modNpcComponent = shared.require(game.ServerScriptService.ServerLibrary.Entity.NpcClass);
-- local modAudio = shared.require(game.ReplicatedStorage.Library.Audio);
-- local DamageData = shared.require(game.ReplicatedStorage.Data.DamageData);

-- local modNekronSpore = shared.require(script:WaitForChild("NekronSpore"));
...
return function(npc, spawnPoint)
	...
	function self.Initialize()
		...
		self.CustomHealthbar.OnDeath:Connect(function(name, healthInfo)
			...
			elseif name:match("Nekros Spore") then
				...
				self.HealthComp:TakeDamage(DamageData.new{
					Damage=amount;
	...
	function self.Update()
		...
		local toxicStatus = self.StatusComp:GetOrDefault("ToxicMod");
		...
		-- if healEffectiveness > 0 then
		-- 	local addNewHealth = math.ceil(self.BaseHealth *0.02 * healEffectiveness);
		...
		-- 	local newHealData = DamageData.new{
		-- 		Damage=-addNewHealth;
		-- 		TargetPart=randomPlaquePart;
		-- 	};
		-- 	self.HealthComp:TakeDamage(newHealData);
