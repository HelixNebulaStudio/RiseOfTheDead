...
local modInterface = shared.require(mainInterface:WaitForChild("InterfaceModule") :: ModuleScript);
...
-- MARK:
local function LobbyInterfaceRequest(lobbyData)
...
modData.LobbyInterfaceRequest = LobbyInterfaceRequest;

-- !outline: signal    bindOpenLobbyInterface.Event
bindOpenLobbyInterface.Event:Connect(LobbyInterfaceRequest)
			...
			button.MouseButton1Click:Connect(function()
				...
				task.delay(0.5, function()
					if workspace:GetAttribute("GameModeComplete") then
						...
						local promptWindow = modClientGuis.promptQuestion("Leave ".. worldName .."?", "Are you sure you want to leave?");
