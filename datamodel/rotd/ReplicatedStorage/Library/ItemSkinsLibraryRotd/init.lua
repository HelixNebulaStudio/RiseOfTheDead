...
--==
function modItemSkinsLibrary.onRequire()

    function modItemSkinsLibrary.GetSkinIdFromOldId(oldId)
        oldId = tostring(oldId);

        if modItemSkinsLibrary:Find(oldId) then
            return oldId;
        end

        for itemSkinId, itemSkinLib in pairs(modItemSkinsLibrary:GetAll()) do
    ...
    function modItemSkinsLibrary:FindVariant(skinId, variantId)
        if skinId == nil or variantId == nil then return end;
        local lib = modItemSkinsLibrary:Find(skinId);

        if lib.Type == modItemSkinsLibrary.SkinType.Pattern then
        ...
        elseif lib.Type == modItemSkinsLibrary.SkinType.Texture then
