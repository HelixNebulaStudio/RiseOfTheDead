...
local modGraphRenderer = shared.require(game.ReplicatedStorage.Library.UI.GraphRenderer);
		...
		if tweakSystemIndex == 1 then
				...
				if tweakId then
					...
					for key, value in pairs(tweaks.Stats) do
						...
						if statLib then
							...
							local statName = STAT_NAMES[key] or key;
			...
			local function updateGraph()
				...
				if graphZoom <= 1 then
					...
					if xBarTweenToggle == true then
						if localPlayer:IsAncestorOf(xBarFrame) then
				...
				for a=2, 5 do
					...
					if value == nil then
						...
						if localPlayer:IsAncestorOf(intPoint) then
			...
			for _, obj in pairs(buttonsFrame:GetChildren()) do
				if obj:IsA("ImageButton") then
					local _isLeft = obj.Name:sub(1,4) == "Left";
