...
function Skill:Trigger(profile, triggerType, points, ...)
	...
	if triggerType == "OnNightTimeStart" then
		local level, stats = Skill:CalStats(Skill.Library, points);
		local duration = math.ceil(stats.Time.Value*60);
		classPlayer:SetProperties("nigowl", {Expires=modSyncTime.GetTime() + duration; Duration=duration; Percent=stats.Percent.Value});
	end
end
