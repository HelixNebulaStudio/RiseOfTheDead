...
local StageLib = GameLib and modGameModeLibrary.GetStage("Raid", "Tombs");
...
local Stages = {
	...
	{ -- Stage 4
		...
		Duration=15;
		Enemies={
			{Name="Zombie"; Chance=0.4};
			{Name="Ticks Zombie"; Chance=0.3};
			{Name="Leaper Zombie"; Chance=0.3};
		};
		UnlockDoors={"StageDoor4"};
	};
	{ -- Stage 5
		UsePrevStageSpawns = false;
		Spawns={};
...
--== Script;
local respawnDoorCache = {};
local activeDoors = {};
local startDoorPrefab = stageElements:WaitForChild("StartDoors"):WaitForChild("StartDoor");
startDoorPrefab.Parent = game.ServerStorage;

for _, obj in pairs(stageElements:WaitForChild("Doors"):GetChildren()) do
	table.insert(respawnDoorCache, obj);
...
function GameController:Hud(data)
	remoteGameModeHud:FireAllClients({
		...
		Type="Raid";
		Stage="Tombs";
		Header=data.Header or "Raid: Tombs";
		Status=data.Status;
...
function GameController:StartStage(stage)
	...
	if stage >= 6 then
		...
		for a=#self.Players, 1, -1 do
			if not self.Players[a]:IsDescendantOf(game.Players) then
				...
				modMission:Progress(self.Players[a], 40, function(mission)
		...
		modItemDrops.Spawn({Type="Tool"; ItemId=StageLib.RewardsId}, CFrame.new(rewardSpawn.Position), self.Players, false);
	...
	if stageInfo.UnlockDoors then
		...
		for a=1, #stageInfo.UnlockDoors do
			for _, obj in pairs(stageElements.Doors:GetChildren()) do
				if obj.Name == stageInfo.UnlockDoors[a] then
					local modDestructible = require(obj:WaitForChild("Destructible"));
					modDestructible:SetEnabled(true);
				end
			end
		end
	end
	...
	repeat
		...
		elseif #GameController.Enemies > 15 then
			spawnChance = 0.45;
		elseif #GameController.Enemies > 10 then
			spawnChance = 0.5;
		end
		...
		if workspace:GetAttribute("NextArenaStage") then
			workspace:SetAttribute("NextArenaStage");
			break;
		end
		wait(1);
...
function GameController:Start()
	...
	for _, obj in pairs(activeDoors) do
		obj:Destroy();
	end
	activeDoors = {};
	
	for _, obj in pairs(stageElements.StartDoors:GetChildren()) do
		local modDestructible = require(obj.Destructible);
		modDestructible:SetEnabled(true);
		table.insert(activeDoors, obj);
	end
	
	
	for a=1, #respawnDoorCache do
		local new = respawnDoorCache[a]:Clone();
		new.Parent = stageElements.Doors;
		table.insert(activeDoors, new);
		
		local modDestructible = require(new:WaitForChild("Destructible"));
		modDestructible:SetEnabled(false);
		local desFunc = modDestructible.OnDestroy;
		function modDestructible:OnDestroy()
...
function OnCharacterAdded(character)
	...
	classPlayer:OnNotIsAlive(function(character)
		...
		if #GameController.Characters <= 0 and GameController.Status == enumGameStatus.InProgress then
			...
			local new = startDoorPrefab:Clone();
			new.Parent = stageElements.StartDoors;
			local modDestructible = require(new:WaitForChild("Destructible"));
			modDestructible:SetEnabled(false);
			wait(5);
