...
local zriceraModel = script:WaitForChild("Prefab").Value; 
local humanoid = zriceraModel:WaitForChild("Zombie");
local rootPart = zriceraModel:WaitForChild("HumanoidRootPart");
...
parentChangeSignal = zriceraModel:GetPropertyChangedSignal("Parent"):Connect(function()
	if zriceraModel.Parent ~= nil then return end
		...
		if Tracks[tracks[a]] == nil then
			...
			Tracks[tracks[a]] = tracks[a]:GetMarkerReachedSignal("Step"):Connect(function(paramString)
				...
				if paramString == "1" then
					foundTrack = tracks[a];
					if zriceraModel:FindFirstChild("RightHand") then
						modAudio.Play("HeavyThump", zriceraModel.RightHand).PlaybackSpeed = random:NextNumber(0.65, 1);
					end
					screenShake();
				elseif paramString == "2" then
					foundTrack = tracks[a];
					if zriceraModel:FindFirstChild("LeftHand") then
						modAudio.Play("HeavyThump", zriceraModel.LeftHand).PlaybackSpeed = random:NextNumber(0.65, 1);
					end
					screenShake();
				end
			end)
