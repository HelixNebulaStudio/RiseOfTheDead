...
function Zombie.new(self)
					...
					elseif doorModule and doorModule:IsA("ModuleScript") then
						...
						if doorObject.Open ~= true and doorObject.CanBreakIn ~= false then
							if self.BreakDoorRandom:PrdRandom(nil, breakDoorChance) then
								...
								--modAudio.Play("MetalSlam", hitPart).PlaybackSpeed = 1;
							else
								doorObject:PlaySlam(random:NextNumber(1.3, 1.6));
								--modAudio.Play("MetalSlam", hitPart).PlaybackSpeed = random:NextNumber(1.3, 1.6);
							end
						end
	...
	return function(targetHumanoid, paramPacket)
		...
		if targetHumanoid then
			...
			local dmgRatio = math.pow((1-math.clamp(distance/self.Properties.AttackRange, 0, 1)), 1/2);
			local attackDamage = self.Properties.AttackDamage * dmgRatio - (self.DamageReduction or 0);
