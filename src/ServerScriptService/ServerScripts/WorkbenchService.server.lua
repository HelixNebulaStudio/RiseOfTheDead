...
--function remoteCheckBlueprintFulfillment.OnServerInvoke(player, interactPart, bpId)
...
--	local fulfillment = OnBlueprintFulfillmentCheck(player, bpId);
--	modOnGameEvents:Fire("OnBlueprintFulfillmentCheck", player, fulfillment);
--	debounceCache[player.Name]=nil;
--	return fulfillment;
--end;

--function remoteBlueprintHandler.OnServerInvoke(player, interactPart, action, parm4)
--	if debounceCache[player.Name] and tick()-debounceCache[player.Name]<0.3 then return modWorkbenchLibrary.BlueprintReplies.TooFrequentRequest end;
--	debounceCache[player.Name]=tick();
--	--if not IsInWorkbenchRange(player, interactPart) then return modWorkbenchLibrary.BlueprintReplies.TooFar; end;
--	--local profile = modProfile:Get(player);
--	--local activeSave = profile:GetActiveSave();
--	--local inventory = profile.ActiveInventory;
...
--	--				shared.Notify(player, ("-$Amount $Stat."):gsub("$Amount", r.Amount):gsub("$Stat", r.Name), "Negative");
--	--			end
--	--			activeSave:AddStat(r.Name, -r.Amount);
...
--	if action == 7 then -- Skip Build;
--		--local buildIndex = parm4;

--		--local activeBp = userWorkbench:GetProcess(buildIndex);
--		--if activeBp and activeBp.BT > modSyncTime.GetTime() then
--		--	local itemId = activeBp.ItemId;
--		--	local bpLib = modBlueprintLibrary.Get(itemId);

--		--	local skipCost = modWorkbenchLibrary.GetSkipCost(activeBp.BT-modSyncTime.GetTime());

--		--	if activeSave and activeSave:GetStat("Perks") >= skipCost then
--		--		activeSave:AddStat("Perks", -skipCost);
--		--		modAnalytics.RecordResource(player.UserId, skipCost, "Sink", "Perks", "Gameplay", "SkipBuild");
...
--	elseif action == 4 then -- Custom build
--		local itemId = parm4;
--		local bpLib = modBlueprintLibrary.Get(itemId);

--		if bpLib and userBlueprints:IsUnlocked(itemId) then
--			--if bpLib.Disabled == true then return modWorkbenchLibrary.BlueprintReplies.Disabled end;
--			--if not userWorkbench:CanNewProcess() then return modWorkbenchLibrary.BlueprintReplies.WorkbenchFull end;

--			--local fulfilment = OnBlueprintFulfilmentCheck(player, itemId);
--			--if fulfilment == nil then return modWorkbenchLibrary.BlueprintReplies.InvalidBlueprint end;
--			--for _, r in pairs(fulfilment) do
--			--	if not r.Fulfilled then return modWorkbenchLibrary.BlueprintReplies.InsufficientCurrency end;
--			--end;
...
--			--local buildTime = modSyncTime.GetTime()+buildDuration;
--			--userWorkbench:NewProcess{
--			--	Type=userWorkbench.ProcessTypes.Building;
--			--	ItemId=itemId;
--			--	BT=buildTime;
...
--			return modWorkbenchLibrary.BlueprintReplies.Success;
--		else
--			Debugger:Log("Failed to build blueprint ("..itemId..")");
--			return modWorkbenchLibrary.BlueprintReplies.BlueprintLocked;
--		end

--	elseif action == 5 then -- Cancel
--		local buildIndex = parm4;
--		--local buildingData = userWorkbench:GetProcess(buildIndex);
--		--if buildingData == nil then return modWorkbenchLibrary.BlueprintReplies.InvalidBlueprint; end;
...
--		--	local requirement = blueprintLibrary.Requirements[a];
--		--	if requirement.Type == "Item" then
--		--		table.insert(itemsRefund, {ItemId=requirement.ItemId; Data={Quantity=(requirement.Amount or 1)}})
--		--	end
--		--end
--		--local enoughSpaceForRefund = inventory:SpaceCheck(itemsRefund);
--		--if enoughSpaceForRefund then
--		--	for a=1, #blueprintLibrary.Requirements do
--		--		local requirement = blueprintLibrary.Requirements[a];
--		--		if requirement.Type == "Stat" then
--		--			if requirement.Name == "Perks" or requirement.Name == "Money" then
--		--				activeSave:AddStat(requirement.Name, requirement.Amount);
--		--			end
--		--		end
--		--	end
--		--	for a=1, #itemsRefund do
...
--		--		local itemLib = modItemsLibrary:Find(item.ItemId);
--		--		inventory:Add(item.ItemId, {Quantity=(item.Data and item.Data.Quantity or 1);}, function(event, storageItem)
--		--			shared.Notify(player, (storageItem.Quantity) > 1 and itemLib.Name.." ("..storageItem.Quantity..")" or itemLib.Name, "PickUp");
--		--			modStorage.OnItemSourced:Fire(nil, storageItem,  storageItem.Quantity);
--		--		end);
--		--	end
--		--	userWorkbench:RemoveProcess(buildIndex);
--		--	--userBlueprints:RemoveBuild(buildIndex);
--		--else
--		--	return modWorkbenchLibrary.BlueprintReplies.InventoryFull;
--		--end
--		--return modWorkbenchLibrary.BlueprintReplies.Success;

--	elseif action == 6 then -- Claim
--		local buildIndex = parm4;

--		--local activeBp = userWorkbench:GetProcess(buildIndex);
--		--if activeBp and activeBp.BT-modSyncTime.GetTime() <= 0 then
--		--	local itemId = activeBp.ItemId;
--		--	local blueprintLibrary = modBlueprintLibrary.Get(itemId);

--		--	if inventory:SpaceCheck{{ItemId=blueprintLibrary.Product; Data={Quantity=(blueprintLibrary.Amount or 1)};}} then
--		--		local itemLib = modItemsLibrary:Find(blueprintLibrary.Product);
--		--		inventory:Add(blueprintLibrary.Product, {Quantity=(blueprintLibrary.Amount or 1);}, function(event, storageItem)
--		--			Debugger:Warn("Build claim ", storageItem);
--		--			shared.Notify(player, string.gsub("You recieved a $Item.", "$Item", itemLib.Name), "Reward");
--		--			modStorage.OnItemSourced:Fire(nil, storageItem,  storageItem.Quantity);
--		--		end);
...
--		--	return modWorkbenchLibrary.BlueprintReplies.Success;
--		--end
--		return modWorkbenchLibrary.BlueprintReplies.TooFrequentRequest;

--	else -- Building by blueprint item;
--		local id = parm4;
--		local storageItem = inventory ~= nil and inventory:Find(id) or nil;
--		local itemId = storageItem and storageItem.ItemId or nil;
--		local bpLib = storageItem and modBlueprintLibrary.Get(storageItem.ItemId) or nil;

--		if storageItem and bpLib then
--			if action == 1 then -- Build
--				--if bpLib.Disabled == true then return modWorkbenchLibrary.BlueprintReplies.Disabled end;
--				--if not userWorkbench:CanNewProcess() then return modWorkbenchLibrary.BlueprintReplies.WorkbenchFull end;

--				--local fulfillment = OnBlueprintFulfillmentCheck(player, itemId);
--				--if fulfillment == nil then return modWorkbenchLibrary.BlueprintReplies.InvalidBlueprint end;
--				--for _, r in pairs(fulfillment) do
--				--	if not r.Fulfilled then return modWorkbenchLibrary.BlueprintReplies.InsufficientCurrency end;
--				--end;
--				--consumeRequirements(fulfillment);
				
--				--inventory:Remove(id, 1, function()
--				--	shared.Notify(player, string.gsub("$Item removed from your Inventory.", "$Item", storageItem.Name), "Negative");
--				--end);

--				--local buildDuration = bpLib.Duration;
--				--modSkillTree:TriggerSkills(player, "OnBlueprintBuild", buildDuration, function(newDuration)
--				--	buildDuration = newDuration;
--				--end);

--				---- new blueprints processes;
--				--local buildTime = modSyncTime.GetTime()+buildDuration;
--				--userWorkbench:NewProcess{
--				--	Type=userWorkbench.ProcessTypes.Building;
--				--	ItemId=itemId;
...
--	end
--	debounceCache[player.Name]=nil;
--	return modWorkbenchLibrary.BlueprintReplies.InvalidBlueprint;
--end
