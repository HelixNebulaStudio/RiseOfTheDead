...
local UpdateFreecam do
	...
	function UpdateFreecam(camera, dt)
		...
		velSpring.t = Vector3.new(dx, dy, dz) * SpeedModifier * camSpeed;
...
local function EnterFreecam()
	print("FreeCamera>> Enter Freecam");
	characterProperties.CharacterCameraEnabled = false;
	characterProperties.FreecamState = 1;
	player:SetAttribute("FreecamState", 1);
	...
	Maid:Mark(UIS.InputBegan:Connect(function(input, processed)
		if input.UserInputType == Enum.UserInputType.MouseButton2 then
			...
			player:SetAttribute("FreecamState", 2);
			characterProperties.FreecamState = 2;
			characterProperties.CanMoveFreeCam = false;
			...
			isPanning = false;
			player:SetAttribute("FreecamState", 1);
			characterProperties.CanMoveFreeCam = true;
	...
	--proxyCamera.CameraType = Enum.CameraType.Scriptable
	...
	fovSpring.t, fovSpring.v, fovSpring.x = currentCamera.FieldOfView, 0, currentCamera.FieldOfView
...
local function ExitFreecam()
	print("FreeCamera>> Exit Freecam");
	characterProperties.CharacterCameraEnabled = true;
	characterProperties.CanMoveFreeCam = true;
	characterProperties.FreecamState = 0;
	player:SetAttribute("FreecamState", 0);
	...
	--proxyCamera.FieldOfView = DEF_FOV
	--proxyCamera.CameraType = Enum.CameraType.Scriptable
...
if player.UserId == 16170943 or player.UserId <= 0 then 
	--key = Enum.KeyCode.LeftCurly;
end;

game:GetService("ContextActionService"):BindActionAtPriority("FreecamToggle", function(actionName, inputState, inputObject)
	if inputState == Enum.UserInputState.Begin then
		if UIS:IsKeyDown(Enum.KeyCode.LeftShift) or UIS:IsKeyDown(Enum.KeyCode.RightShift) then
			if freeCamEnabled then
				...
				if modConfigurations.AllowFreecam ~= true then
					warn("FreeCamera>> Freecam not allowed.");
					return;
				end
...
modConfigurations.OnChanged("AllowFreecam", function()
	if modConfigurations.AllowFreecam ~= true then
		ExitFreecam();
	end
end)
