						...
						while checkLoop do
							...
							if lastSpawnTick == nil or tick() > lastSpawnTick then
								...
								if profile and classPlayer and classPlayer.IsAlive and classPlayer.Properties.InBossBattle == nil then
									...
									modNpc.Spawn("Bandit", profile.LastDoorCFrame, function(npc, npcModule)
							...
							if classPlayer.IsAlive == false then
								task.delay(1, function()
									modMission:FailMission(player, missionId, "You died and the high value package was stolen.");
								end)
								endMission();
								break;
							end
