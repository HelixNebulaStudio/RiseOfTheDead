...
for a, track in pairs(Animator:GetPlayingAnimationTracks()) do
	track:Stop();
end
...
local animNames = { 
	...
	walk = 	{ 	
				{ id = "http://www.roblox.com/asset/?id=913402848", weight = 10 }  --Walk
			}, 
	run = 	{
				{ id = "http://www.roblox.com/asset/?id=913376220", weight = 10 }  --Sprint
			}, 
	swim = 	{
...
script:WaitForChild("PlayEmote").OnInvoke = function(emote)
	...
	local animator = Humanoid:WaitForChild("Animator");
	if animator then
		local activeAnims = animator:GetPlayingAnimationTracks();
		for k, v in pairs(activeAnims) do
			if tostring(v) == emote then
				
				currentlyPlayingEmote = false;
				
				playAnimation("idle", 0.2, animator)
				pose = "Standing"
	...
	if typeof(emote) == "Instance" and emote:IsA("AnimationTrack") then
	...
	local animLib = modEmotes:Find(emote);
	if animLib == nil then warn("Animate>>  Unknown emote "..emote) return end;
	emoteNames[animLib.Id] = false; --animLib.Looped == true;
	--if looped then emoteNames[animLib.Id] = true; end;
	
	if animLib then
		playEmote(animLib.Animation, EMOTE_TRANSITION_TIME, animator)
		return true
		
	elseif emoteNames[emote] ~= nil then
		-- Default emotes
		playAnimation(emote, EMOTE_TRANSITION_TIME, Humanoid)
