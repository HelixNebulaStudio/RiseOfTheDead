...
function Mod.Activate(packet)
	...
	local sLayerInfo = modModsLibrary.GetLayer("S", packet);
...
if RunService:IsServer() then
	...
	function Mod.IceBlast(frostStack, npcModule, params)
		local player = frostStack.Player;
	...
	function Mod.ActivateMod(damageSource)
		...
		if frostStack == nil then
			...
			Mod.NewFrostStack(npcModule, frostStack, damageSource, player);
		end
		...
		if frostStack.DamagePool >= healthPerStage and tick() - frostStack.LastStackTick >= 0.1 and frostStack.CompleteTick == nil then
			...
			if frostStack.Stacks >= maxStacks then
				...
				task.delay(0.1, function()
					Mod.IceBlast(frostStack, npcModule, {Targets=maxTargets; Radius=maxRadius;});
				end)
			end
