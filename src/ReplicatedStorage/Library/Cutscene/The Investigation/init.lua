...
return function(CutsceneSequence)
	if modBranchConfigs.IsWorld("TheResidentials") then
		CutsceneSequence:Initialize(function()
			...
			local mission = modMission:GetMission(player, 52);
	...
	elseif modBranchConfigs.IsWorld("TheInvestigation") then
		...
		local robertLeftHand = script:WaitForChild("RobertLeftHand");
		local cutStrap = script:WaitForChild("cutStrap");
			...
			local function OnChanged(firstRun)
				...
				elseif mission.Type == 1 then -- OnActive
					...
					elseif mission.ProgressionPoint == 5 then
						robertModule.SetAnimation("Punch", {script.Punch});
						robertModule.SetAnimation("RobertLoseHand", {script.RobertLoseHand});
						robertModule.SetAnimation("RobertInvestigate1", {script.RobertInvestigate1});
						josephModule.SetAnimation("JosephInvestigate1", {script.JosephInvestigate1});
						josephModule.SetAnimation("JosephDown", {script.JosephDown});
						josephModule.SetAnimation("JosephDown2", {script.JosephDown2});
						josephModule.SetAnimation("JosephNoArm", {script.JosephNoArm});
						zarkModule.SetAnimation("LookAround", {script.LookAround});
						zarkModule.SetAnimation("CrouchLookAnim", {script.CrouchLookAnim});
					...
					elseif mission.ProgressionPoint == 7 then
						...
						local new = script.rockDebris:Clone();
					...
					elseif mission.ProgressionPoint == 8 then
						...
						local newWeld = script.RobertLeftHandWeld:Clone();
					...
					elseif mission.ProgressionPoint == 15 then
						local new = script:WaitForChild("Travel_TheMall"):Clone();
