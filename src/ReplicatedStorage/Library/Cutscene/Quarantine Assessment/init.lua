		...
		elseif triggerTag == "QA2_Radio" then
			...
			if mission then
				...
				elseif mission.ProgressionPoint == 4 then
					...
					local radioMsg = "[Radio] Commander: Wilson is still alive?! Hello? This is commander Leo. Please state your location.";
					interactData.Label = radioMsg;
					shared.Notify(player, radioMsg, "Message");
					interactData:Sync();
...
return function(CutsceneSequence)
	if modBranchConfigs.IsWorld("TheUnderground") then
		local templateRadioInteractable = script:WaitForChild("militaryRadioInteractable");
		
		CutsceneSequence:Initialize(function()
			local players = CutsceneSequence:GetPlayers();
			local player = players[1];
			local mission = modMission:GetMission(player, 51);
			if mission == nil then return end; 

			if not modMission:IsComplete(player, 51) then
				...
				local function OnChanged(firstRun)
					...
					elseif mission.Type == 1 then -- OnActive
						if mission.ProgressionPoint == 1 then
							newInteractable = templateRadioInteractable:Clone();
	...
	elseif modBranchConfigs.IsWorld("TheResidentials") then
		local templateRadioInteractable = script:WaitForChild("militaryRadioInteractable2");
