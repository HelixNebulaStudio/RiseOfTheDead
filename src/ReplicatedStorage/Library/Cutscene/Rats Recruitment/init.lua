...
return function(CutsceneSequence)
	if modBranchConfigs.IsWorld("TheHarbor") then
		CutsceneSequence:Initialize(function()
			...
			local mission = modMission:GetMission(player, 62);
	...
	elseif modBranchConfigs.IsWorld("SectorE") then
		...
		local mechDoor3Clip = script:WaitForChild("_playerClip") ;
		local terminalInteractable = script:WaitForChild("shutDownHeating");
			...
			local function OnChanged(firstRun)
				...
				elseif mission.Type == 1 then -- OnActive
					...
					elseif mission.ProgressionPoint == 5 then
						eugeneModule.Interactable.Parent = script;
						...
						local playersInRegion = modRegion:GetPlayersWithin(script.ChamberRegion);
