...
local triggerLightBarrel = script:WaitForChild("lightFirebarrel");
local bmSpawn = CFrame.new(-26.1658859, 2.54966354, -114.670898, -1, 0, 0, 0, 1, 0, 0, 0, -1);
...
return function(CutsceneSequence)
	if not modBranchConfigs.IsWorld("EasterButchery") then
		...
		CutsceneSequence:Initialize(function()
			...
			local mission = modMission:GetMission(player, 50);
			if mission == nil then return end;

			if not modMission:IsComplete(player, 50) then
	...
	CutsceneSequence:Initialize(function()
		...
		local bunnymanModule = modNpc.GetPlayerNpc(player, "Bunny Man");
		if bunnymanModule == nil then
			local npc = modNpc.Spawn("Bunny Man", bmSpawn, function(npc, npcModule)
			...
			local function OnChanged(firstRun)
				...
				elseif mission.Type == 1 then -- OnActive
					...
					elseif mission.ProgressionPoint == 6 then
						...
						for a=1, 3 do
							local projectileObject = modProjectile.Fire("firework", origin, Vector3.new());
						...
						modNpc.Spawn("Cultist", CFrame.new(-59.21, 2.53, -8.22, 0, 0, 1, 0, 1, 0, -1, 0, 0), function(npc, npcModule)
						...
						spawn(function()
							...
							while true do
								modNpc.Spawn("Cultist", cultspawns[math.random(1, #cultspawns)].CFrame, function(npc, npcModule)
