...
return function(CutsceneSequence)
	CutsceneSequence:Initialize(function()
		...
		local mission = modMission:GetMission(player, 42);
		...
		if not modBranchConfigs.IsWorld("VindictiveTreasure") and mission.ProgressionPoint >= 2 then
			if mission.ProgressionPoint >= 1 then
				...
				modMission:FailMission(player, 42, "You left Victor behind..");
			end
			
			if modEvents:GetEvent(player, "mission42Bp2") == nil and mission.Type == 3 then
				local dropPrefab = modItemDrops.Spawn(
					{Type=modItemDrops.Types.Blueprint; ItemId="tacticalbowbp";}, 
					classPlayer.RootPart.CFrame, {player}, false);
			end
		end
		...
		local victorModule;
		modNpc.Spawn("Victor", victorSpawn, function(npc, npcModule)
			npcModule.Owner = player;
			victorModule = npcModule;
			victorModule.RootPart.Anchored = false;
		end);
		victorModule.SetAnimation("PushButton", {script.PushButton});
		victorModule.SetAnimation("LookDown", {script.LookDown});
		victorModule.SetAnimation("Push", {script.Push});
		victorModule.SetAnimation("Focus", {script.Focus});
		victorModule.SetAnimation("Stuck", {script.Stuck});
		...
		script:WaitForChild("BridgeCFrameTween").Value = bridgeDefaultCF;
		script.BridgeCFrameTween:GetPropertyChangedSignal("Value"):Connect(function()
			cutsceneElements.Bridge:SetPrimaryPartCFrame(script.BridgeCFrameTween.Value);
		end)
		
		local nekronMask = script:WaitForChild("Nekron Mask");
		...
		if not modMission:IsComplete(player, 42) then
			...
			local function OnChanged(firstRun)
				...
				elseif mission.Type == 1 then -- OnActive
					...
					elseif mission.ProgressionPoint == 2 then
						...
						script.BridgeCFrameTween.Value = bridgeDefaultCF;
					...
					elseif mission.ProgressionPoint == 3 then
						...
						table.insert(tweenObjs, 
							TweenService:Create(cutsceneElements.firstGate, TweenInfo.new(5), {Position=Vector3.new(-88.706, 129.802, -70.402)})
							)
					...
					elseif mission.ProgressionPoint == 4 then
						...
						for a=1, 10 do
							modNpc.Spawn("Zombie", zombieSpawn, function(npc, npcModule)
					...
					elseif mission.ProgressionPoint == 5 then
						...
						if activeWeakWall then
							local destructible = require(activeWeakWall.Destructible);
					...
					elseif mission.ProgressionPoint == 8 then
						...
						if modEvents:GetEvent(player, "mission42Bp2") == nil then
							local dropPrefab = modItemDrops.Spawn(
								{Type=modItemDrops.Types.Blueprint; ItemId="tacticalbowbp";}, 
								blueprintSpawnCf, {player}, false);
						end
					end
				elseif mission.Type == 3 then -- OnComplete
					local vtExit = script:WaitForChild("Travel_TheWarehouse"):Clone();
					modReplicationManager.ReplicateIn(player, vtExit, workspace.Interactables);
