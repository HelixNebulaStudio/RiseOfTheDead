	...
	if modBranchConfigs.IsWorld("TheWarehouse") then
		modOnGameEvents:ConnectEvent("OnTrigger", function(player, interactData)
			...
			if triggerTag == "MB2 Note" then
				if not modMission:IsComplete(player, 44) then
					modMission:StartMission(player, 44);
...
return function(CutsceneSequence)
	...
	if modBranchConfigs.IsWorld("TheWarehouse") then
		CutsceneSequence:Initialize(function()
			...
			local mission = modMission:GetMission(player, 44);
			...
			if not modMission:IsComplete(player, 44) then
				...
				local function OnChanged(firstRun)
					if mission.Type == 2 then -- OnAvailable
						local noteTrigger = script:WaitForChild("noteTrigger"):Clone();
						noteTrigger.Parent = workspace.Interactables;
						modReplicationManager.ReplicateOut(player, noteTrigger);

					elseif mission.Type == 1 then -- OnActive
						if mission.ProgressionPoint == 1 then
							local footsteps = script:WaitForChild("warehouseFootprints"):Clone();
		...
		CutsceneSequence:Initialize(function()
			...
			if not modMission:IsComplete(player, 44) then
				...
				local function OnChanged(firstRun)
					...
					elseif mission.Type == 1 then -- OnActive
						if mission.ProgressionPoint == 1 then
							local footsteps = script:WaitForChild("residentialsFootprints"):Clone();
							footsteps.Parent = workspace.Environment;
							modReplicationManager.ReplicateOut(player, footsteps);

							local jacksPhone = script:WaitForChild("jacksPhone"):Clone();
							jacksPhone.Parent = workspace.Interactables;
							modReplicationManager.ReplicateOut(player, jacksPhone);

							local phoneTouchTrigger = script:WaitForChild("phoneTrigger"):Clone();
						...
						elseif mission.ProgressionPoint == 2 then
							local trapTrigger = script:WaitForChild("trapTrigger"):Clone();
							trapTrigger.Parent = workspace.Interactables;
							modReplicationManager.ReplicateOut(player, trapTrigger);
							
						elseif mission.ProgressionPoint == 3 then
							local trapDoor = script:WaitForChild("trapDoor"):Clone();
				...
				if modEvents:GetEvent(player, "mb2_scythepickup") == nil then
					local freeScythe = script:WaitForChild("freeScythe"):Clone();
					freeScythe.Parent = workspace.Interactables;
					modReplicationManager.ReplicateOut(player, freeScythe);
				end
				
				local basementDoor = script:WaitForChild("basementSecondDoor"):Clone();
