...
LogicTree.Types = {
	Process = function(tree, node)
		if tree.Disabled == true then
			return LogicTree.Status.Failure;
		end;
		
		node.Status = LogicTree.Status.Running;
		return node.Type(tree, node);
	end;
	
	And = function(tree, parentNode)
		...
		if status == LogicTree.Status.Success then
			parentNode.Index = parentNode.Index +1;
			
			if parentNode.Index > #parentNode.Children then
				parentNode.Index = nil;
	...
	IfElse = function(tree, parentNode)
		if parentNode.Index == nil then
			parentNode.Index = 1;
		end
	...
	Leaf = function(tree, node)
		...
		if status == nil then
			status = LogicTree.Status.Failure;
		end
		node.Status = status
...
setmetatable(Node, LogicTree);
LogicTree.Node = Node;
--==

function LogicTree.new(treeTable)
	local self = {
		Name = nil;
		Root = Node.new("Root");
		Nodes = {};
	...
	-- Init;
	local function load(parentNode, rawNode)
...
function LogicTree:NewNode(name)
...
function LogicTree:Hook(nodeName, func)
...
function LogicTree:Call(nodeName)
	local node = self.Nodes[nodeName];
