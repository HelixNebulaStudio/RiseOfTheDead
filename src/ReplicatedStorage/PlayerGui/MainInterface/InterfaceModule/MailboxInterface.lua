...
local mailboxFrame = script.Parent.Parent:WaitForChild("MailboxFrame");
local mailboxList = mailboxFrame:WaitForChild("MainFrame"):WaitForChild("MailList");
local mailboxLayout = mailboxList:WaitForChild("UIListLayout");
local mailboxPadding = mailboxList:WaitForChild("UIPadding");

local socialMenuFrame = script.Parent.Parent:WaitForChild("SocialMenu");
local mailboxQuickButton = socialMenuFrame:WaitForChild("Nav"):WaitForChild("Mailbox");
...
function Interface.init(modInterface)
	...
	local window = Interface.NewWindow("Mailbox", mailboxFrame);
	...
	--modKeyBindsHandler:SetDefaultKey("KeyWindowMailbox", Enum.KeyCode.O);
	
	mailboxQuickButton.MouseButton1Click:Connect(function()
		Interface:PlayButtonClick();
		Interface:OpenWindow("Mailbox");
	end)
			...
			elseif template.ActionType == "LegacyCollect" then
				...
				deleteButton.MouseButton1Down:Connect(function()
					...
					RunService:BindToRenderStep("DeleteConfirm", Enum.RenderPriority.Input.Value+1, function(delta)
						...
						if not Interface.Button1Down then
							RunService:UnbindFromRenderStep("DeleteConfirm");
