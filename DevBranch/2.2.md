...
- Added wall climbing and double jumping ability to `ninjacloak` passive `Ninja Agility`.
...
- Updated `brownleatherboots`. No longer gives 10%. Now has passive, `Bull Leaping`, Slide jumping adds 10 units/sec launch speed.
...
- Added Chest and Head clothing, `ninjacloak` and `ninjashroud`.
    - Chest clothing, `ninjacloak` has passive `Ninja Agility`. Ability WIP.
    - Head clothing, `ninjashroud` has passive `Ninja Focus`. Ability WIP.
> 2.1.24.10
