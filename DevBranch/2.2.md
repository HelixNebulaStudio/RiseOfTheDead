...
- On mission 2, `Where Am I`. Mission point 6, will now set players gun ammo to 0. This is for a common case where players who didn't shoot a single round in the beginning not showing a refill option in shop, causing confusion.
...
- Entering commands are now no longer using TextChatService due to moderation complications. Sending a message that begins with `/` will automatically change input box's font and color to Code & yellow.
